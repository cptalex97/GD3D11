...
XRESULT D3D11PFX_HeightFog::Render( RenderToTextureBuffer* fxbuffer ) {
	...
	cb.HF_GlobalDensity = GoucWeatherFogDensity( Engine::GAPI->GetRendererState().RendererSettings.FogGlobalDensity );
	...
	// GOUC: a weather fog pulls the distance ramp in, otherwise it only thickens hundreds of
	// metres out and the near field stays clear -- which is where one actually looks.
	{
