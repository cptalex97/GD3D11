...
// t5 war TX_ReflectionCube (Nass-Glanz, entfernt -- s. ApplySceneWettness)
...
void ApplySceneWettness(float3 wsPosition, float3 vsPosition, inout float3 vsNormal, in out float3 diffuse, in out float specIntensity, in out float specPower, out float localWettness)
{
	...
	// GOUC 23.09.2026: der zusaetzliche Nass-Glanz ist entfernt (Upstream kirides ffb4f773, #506/#508).
	// Er bestand aus einer Himmelswuerfel-Spiegelung und drei festen Kunstlichtern. Das erste Licht
	// stand im KAMERA-Raum (nicht wie die anderen beiden in den View umgerechnet), und der Wuerfel
	// wurde mit einem View-Space-Vektor abgetastet -- beide Glanzpunkte hingen damit an der Kamera
	// und drehten sich im Spiel als helle Punkte um den Charakter mit. Die Naesse bleibt: dunklere,
	// entsaettigte Oberflaeche und die Normalenverformung durch den Regen.
...
float4 PSMain(PS_INPUT Input) : SV_TARGET
{
    ...
    ApplySceneWettness(wsPosition, vsPosition, normal, diffuse.rgb, specIntensity, specPower, localWettness);
#endif
